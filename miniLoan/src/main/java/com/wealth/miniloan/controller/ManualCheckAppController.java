...
public class ManualCheckAppController extends BaseController {
	...
	public ModelAndView toCheckApp(MlAppSummary appSummary) {
		ModelAndView modelAndView = new ModelAndView();
		String appType = appSummary.getAppType();

		MlAppCheckResultExample example = new MlAppCheckResultExample();
		Criteria c = example.createCriteria();
		c.andAppNoEqualTo(appSummary.getAppNo());
		c.andCheckTypeEqualTo("人工复核");
		Object obj = this.appSummaryService.getByExample(example);
		MlAppCheckResult acr = (MlAppCheckResult) obj;
		if ("01".equals(appType)) {
			modelAndView.setViewName("check/naturalAppDetail");
			MlNaturalApp app = new MlNaturalApp();
			app.setAppNo(appSummary.getAppNo());
			...
			app.setAppNo(appSummary.getAppNo());
			MlCorpApp ca = this.corpAppService.getByPriKey(app);
			modelAndView.addObject("checkApp", ca);
		}
		modelAndView.addObject("checkResult", acr);
		modelAndView.addObject("flag", "MANUALCHECK");
		return modelAndView;
	}
