...
<script type="text/javascript">
	...
	$(function() {
		$('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/blacknames/getNameList.do?nameType=02',
			fit : true,
			fitColumns : true,
			striped : true,
			border : false,
	...
	function addNew() {
		window.location.href = '${pageContext.request.contextPath}/blacknames/toAddName.do';
	...
	function updateObj() {
		...
		if (rows.length > 0) {
			if (rows.length == 1) {
				window.location.href = '${pageContext.request.contextPath}/blacknames/toUpdateName.do?nameId='
						+ rows[0].nameId;
			} else {
		...
		if (rows.length > 0) {
			$.messager.confirm('信息提示', '您确认要删除当前选中的记录？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/blacknames/deleteName.do',
						data : {
							ids : ids.join(',')
						},
						type : "POST",
