...
<script type="text/javascript">
	...
	$(function() {
		$('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/whitenames/getNameList.do?nameType=01',
			fit : true,
			fitColumns : true,
			striped : true,
			border : false,
	...
	function addNew() {
		window.location.href = '${pageContext.request.contextPath}/whitenames/toAddName.do';
	...
	function updateObj() {
		...
		if (rows.length > 0) {
			if (rows.length == 1) {
				window.location.href = '${pageContext.request.contextPath}/whitenames/toUpdateName.do?nameId='
						+ rows[0].nameId;
			} else {
			...
			$.messager.confirm('信息提示', '您确认要删除当前选中的记录？', function(r) {
				if (r) {
					for (var i = 0; i < rows.length; i++) {
						ids.push(rows[i].nameId);
					}
					$.ajax({
						url : '${pageContext.request.contextPath}/whitenames/deleteName.do',
						data : {
							ids : ids.join(',')
						},
						type : "POST",
