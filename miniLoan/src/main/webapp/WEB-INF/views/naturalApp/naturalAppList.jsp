...
<script type="text/javascript">
	...
	$(function() {
		$('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/natural/app/naturalAppList.do',
			fit : true,
			fitColumns : true,
			striped : true,
			border : false,
   ...
   function addNew() {
	...
	function editCredit() {
		...
		if (rows.length > 0) {
			if (rows.length == 1) {
				window.location.href = '${pageContext.request.contextPath}/natural/credit/creditDetail.do?appNo='
						+ rows[0].appNo+'&Name='+ rows[0].name;
			} else {
	...
	function editAttach() {
		...
		if (rows.length > 0) {
			if (rows.length == 1) {
				window.location.href = '${pageContext.request.contextPath}/natural/attach/toNaturalAttachList.do?appNo='
						+ rows[0].appNo;
			} else {
				$.messager.alert('信息提示', "只能选择一条要维护的记录！", "info");
			}
