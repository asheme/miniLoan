...
<script type="text/javascript">
	...
	function addNew() {
		window.location.href = '${pageContext.request.contextPath}/mortgage/toAddMortgage.do?appNo=${appNo}';
	...
	function updateObj() {
		...
		if (rows.length > 0) {
			if (rows.length == 1) {
				window.location.href = '${pageContext.request.contextPath}/mortgage/toUpdateMortgage.do?mortgageId='
						+ rows[0].mortgageId;
			} else {
			...
			$.messager.confirm('信息提示', '您确认要删除当前选中的记录？', function(r) {
				if (r) {
					for (var i = 0; i < rows.length; i++) {
						ids.push(rows[i].mortgageId);
					}
					$.ajax({
						url : '${pageContext.request.contextPath}/mortgage/deleteMortgage.do',
						data : {
							ids : ids.join(',')
						},
						type : "POST",
