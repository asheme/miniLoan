...
<title>integrity check cfg item list</title>
<script type="text/javascript">
<!--
	$(function() {
		$('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/integrity/cfg/integrityCfgList.do',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					return getDictItem("INTEGRITY_ITEM_TYPE", value);
				}
			}, {
				field : 'status',
				title : '状态',
	...
	function addNew() {
		window.location.href = '${pageContext.request.contextPath}/integrity/cfg/toAddIntegrityCheckItem.do';
	...
	function updateObj() {
		...
		if (rows.length > 0) {
			if (rows.length == 1) {
				window.location.href = '${pageContext.request.contextPath}/integrity/cfg/toUpdateIntegrityCheckItem.do?itemId='
						+ rows[0].itemId;
			} else {
					...
					.confirm(
							...
							function(r) {
								if (r) {
									...
									$
											.ajax({
												url : '${pageContext.request.contextPath}/integrity/cfg/deleteIntegrityCheckItem.do',
												data : {
													ids : ids.join(',')
												},
												type : "POST",
			...
			<form id="searchForm">
				<table class="querytable" width="100%">
					<tr height="30px;">
						...
						<td width="30%"><input name="itemName" style="width: 275px;"
							class="easyui-validatebox" validType="length[0,20]" /></td>
						<th width="20%">检查项类别</th>
						<td width="30%">
							<input class="easyui-validatebox" id="itemType" name="itemType" style="width: 150px" />	
						</td>
					</tr>
					<tr height="30px;">
